...
namespace Graphics {
	...
	void drawWave		(Mat<ARGB>& image, int* x, int* y, int n);						//画曲线
	...
	void fillRectangle	(Mat<ARGB>& image, int sx, int sy, int ex, int ey, ARGB color);	//填充单色
	void fillFlood		(Mat<ARGB>& image, int x0, int y0, ARGB color);					//泛滥填充
	void fillPolygon	(Mat<ARGB>& image, int* x, int* y, int n);						//多边形填充
